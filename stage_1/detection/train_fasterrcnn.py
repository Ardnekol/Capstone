#!/usr/bin/env python3
"""
Faster R-CNN Training Script for Object Detection

Train Faster R-CNN on TACO dataset and evaluate on both TACO (in-domain) 
and Trash-ICRA19 (cross-domain).

Uses torchvision's implementation of Faster R-CNN with ResNet-50 FPN backbone.

Usage:
    python train_fasterrcnn.py --epochs 50 --batch 4
    python train_fasterrcnn.py --eval --weights checkpoints/best.pth
"""
...
class YOLODataset(Dataset):
    """Dataset for YOLO format annotations (works for both TACO and ICRA19)"""
...
def get_model(num_classes: int):
    ...
    # Replace head with custom number of classes
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, print_freq=50):
    ...
    for i, (images, targets) in enumerate(data_loader):
        ...
        # Skip batch if any target has no boxes (causes issues with Faster R-CNN)
    ...
    for k in loss_dict_accum:
        loss_dict_accum[k] /= len(data_loader)
...
def evaluate(model, data_loader, device):
    ...
    # Compute mAP using COCO-style evaluation
...
def compute_map(predictions: List[Dict], targets: List[Dict], 
    ...
    for iou_thresh in iou_thresholds:
        ...
        for pred, target in zip(predictions, targets):
            ...
            for i in order:
                if pred_scores[i] < 0.01:  # Skip very low confidence
                    continue
...
def train_fasterrcnn(config: dict):
    ...
    output_dir = Path(config['output_dir']) / f"fasterrcnn_{timestamp}"
    ...
    # Create model (num_classes = background + actual classes)
    num_classes = len(config['class_names']) + 1
    model = get_model(num_classes)
    model.to(device)
    
    print(f"Model: Faster R-CNN ResNet50-FPN-v2")
    print(f"Classes: {config['class_names']} ({num_classes} with background)")
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='runs/fasterrcnn')
    ...
    if args.train:
        output_dir, best_map = train_fasterrcnn(config)
        
        # Auto cross-domain evaluation
