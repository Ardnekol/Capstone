#!/usr/bin/env python3
"""
ViT-Base Training Script for Waste Classification

Train ViT-Base on TrashNet dataset and evaluate on both TrashNet and RealWaste.

Usage:
    python train_vit_base.py --epochs 50 --batch 32
"""
...
def create_model(num_classes):
    model = timm.create_model(DEFAULT_CONFIG['model_name'], pretrained=True, num_classes=num_classes)
    return model
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_acc > best_acc:
            best_acc = epoch_acc
            torch.save(model.state_dict(), f'results/vit_base_best.pth')
...
def main():
    parser = argparse.ArgumentParser(description='Train ViT-Base for waste classification')
    ...
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.01)  # AdamW for transformers
    ...
    with open('results/vit_base_results.json', 'w') as f:
        json.dump(results, f, indent=2)

    print("Training complete! Results saved to results/vit_base_results.json")
