...
### 4.5 Field deployment validation on IITH-GVP surveillance imagery

To validate the unified model on actual deployment conditions — not benchmark
test splits — we evaluate it on **IITH-GVP**, a surveillance image dataset
of 46,104 photographs captured at Garbage Vulnerable Points (GVPs) on the
IIT Hyderabad campus between February and May 2024. Each location is
photographed twice per day by fixed surveillance cameras (zones Z5 and Z6,
~22 cameras), yielding paired *before-cleanup* and *after-cleanup* images.
This temporal pairing provides an empirical validation that **requires no
manual labels**: a deployment-quality waste-vision model should detect more
waste in "before" images (locations explicitly chosen because they
accumulate garbage) than in "after" images (the same locations following
municipal cleanup).

**Protocol.** We sample 500 before-cleanup and 500 after-cleanup images
uniformly at random (seed 42), run the unified Florence-2 + LoRA on each
with the `<OD>` (detection) and `<REFERRING_EXPRESSION_SEGMENTATION>waste`
prompts, and aggregate two label-free signals: (1) mean detected
object count per image, and (2) mean foreground area fraction in the
segmentation mask. Both signals should be strictly higher on before
images if the model is correctly recognising waste in this novel
deployment domain.

**Results.** *[TBD — to be filled after `evaluate_iith_gvp.py` finishes]*

| Metric | Before | After | Δ |
|---|---:|---:|---:|
| Mean detected objects per image | _TBD_ | _TBD_ | _TBD_ |
| Mean bbox area fraction | _TBD_ | _TBD_ | _TBD_ |
| Mean segmentation foreground fraction | _TBD_ | _TBD_ | _TBD_ |
| Images with any detection (%) | _TBD_ | _TBD_ | _TBD_ |

Qualitative examples (Figure ?) illustrate the model's predictions on
representative before/after pairs from each zone. The model had never seen
this surveillance data during training: TrashNet (lab photographs) and TACO
(staged outdoor litter) are visually very different from fixed-camera
campus surveillance footage. A positive before-vs-after gap therefore
provides direct evidence that the LoRA fine-tuning generalises to a new
deployment domain without further training.
